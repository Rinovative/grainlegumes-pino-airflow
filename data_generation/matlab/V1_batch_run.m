...
seed       = 999999;        % reproducibility seed
...
template_path = fullfile(project_root, 'comsol', 'V1_template_brinkman.mph');
...
if ~isfile(sample_csv)
    ...
    V1_sample_parameters(method, variation, N, seed, p_log, meta_dir);
else
...
for i = 1:n_cases
    ...
    try
        [kappa, X, Y, info_field] = V1_gen_permeability( ...
        ...
        fprintf('[%4d/%4d] ❌ Error in V1_gen_permeability: %s\n', ...
    ...
    save_model = false;
    fprintf("%s (%.1f s)", field_path)

    try
        [model, results] = V1_run_comsol_case(field_path, template_path, processed_dir, save_model);
